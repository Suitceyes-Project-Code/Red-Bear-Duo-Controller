...
void OnPinSet() {
  Serial.println("Received command via usb");
  byte pin = c.readInt16Arg();
  //byte pin = c.readBinArg<byte>();
  Serial.println("Pin: " + String(pin));
  if (!c.isArgOk()) {
    c.sendCmd(StringMsg, "PinSet: argument pin is not ok.");
    return;
  }
  byte value = c.readInt16Arg();
  //byte value = c.readBinArg<byte>();
  Serial.println("Value: " + String(value));
  if (!c.isArgOk()) {
    c.sendCmd(StringMsg, "PinSet: argument value is not ok.");
    return;
  }

  if (pin >= sizeof(pinsNo)) {
    c.sendCmd(StringMsg, "PinSet: argument pin is out of range.");
  }
